...
SMTP는 두 메일 서버가 먼 거리에 떨어져 있더라도 중간 메일 서버를 이용하지 않는다. 즉, 메시지를 보낼 때 보내는데 실패하더라도 중간 메일 서버에 저장되는 것이 아니라 송신자의 메일 서버에 남아있다.
...
4. 보낼 다른 메시지가 있다면 같은 TCP 연결 상에서 반복. 그렇지 않으면 TCP를 닫는다.
...
클라이언트는 5개의 HELO, MAIL FROM, RCPT TO, DATA, QUIT 명령을 내리고, 클라이언트는 하나의 점으로 된 라인을 송신하며, 그것은 서버에거 메시지의 끝을 나타낸다.

서버는 각명령에 응답하며, 각 응답에는 응답 코드와 영문 설명이 있다.
