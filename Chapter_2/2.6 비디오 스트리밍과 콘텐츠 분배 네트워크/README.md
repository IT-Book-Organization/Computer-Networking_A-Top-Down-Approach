...
- `Bring Home`: Limelight와 다른 회사들에 의해 적용된 것으로 좀 더 적은 수의 핵심 지점에 큰 규모의 서버 클러스터를 구축하여 ISP를 Home으로 가져오는 개념이다. 접속 ISP에 연결하는 대신 일반적으로 CDN들은 그들의 클러스터를 인터넷 교환 지점(IXP)에 배치된다. Enter Deep보다 비용은 적게드나 처리율은 더 낮다.
